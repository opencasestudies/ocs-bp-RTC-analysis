...
knitr::include_graphics("www/img/Lott.png")
```
...
```{r, echo=FALSE, out.height = '100%', out.width = '100%', fig.align='center'}
knitr::include_graphics("www/img/Donohue.png")
```
...
knitr::include_graphics("www/img/Educational_Graphic1.jpg")
```
...
```{r, echo=FALSE, out.height = '100%', out.width = '100%', fig.align='center'}
knitr::include_graphics("www/img/Donohue_Table2_edited.png")
```
...
```{r, echo=FALSE, out.height = '100%', out.width = '100%', fig.align='center'}
knitr::include_graphics("www/img/RTC.png")
```

##### [[source]](https://www.nraila.org/gun-laws/){target="_blank"}

```{r, echo=FALSE, out.height = '100%', out.width = '100%', fig.align='center'}
knitr::include_graphics("www/img/RTC_map.png")
```
...
```{r, echo=FALSE, out.height = '100%', out.width = '100%', fig.align='center'}
knitr::include_graphics("www/img/Donohue_AppendixJ.png")
```
...
```{r, echo=FALSE, out.height = '100%', out.width = '100%', fig.align='center'}
knitr::include_graphics("www/img/ourdata.png")
```
...
```{r, echo=FALSE}
load(file = "www/data/Wrangled_data.rda")
```
...
quiz(caption = "",
  question("Which one of the following terms in the `ggplot()` function is used to specify what data should be plotted on the x-axis and what data should be plotted in on the y-axis?",
...
Var3 <- rnorm(100, -6, 2)
Var2[85] <- NA
mydata <- data.frame(ID, Var1, Var2, Var3)
```

```{r DE_Exercise, exercise = TRUE}
...
# second way
skimr::skim(mydata)
# So there is missing data in "Var2"
```
...
```{r DA_Code1, exercise = TRUE, eval=FALSE}
...
# you can use summary() to check if you got the same model as below
```
...
quiz(caption = "",
  question("Suppose we have a set of data that were collected through measurements of multiple individuals over time, what kind of data do we have?",
    answer("Time series data", message = "Time series data measures one observation over time."),
    answer("Cross sectional data", message = "Cross sectional data measures multiple individuals at one point in time."),
    answer("Panel data", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("In a panel analysis there are N individual panel members and T time points. In an unbalanced panel, we have n observations, where n = N∗T.",
    answer("True", message = "In a balanced panel, we have n observations, where n = N∗T."),
    answer("False", correct = TRUE, message = "In a balanced panel, we have n observations, where n = N∗T."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("Which sub-type of panel regression analysis should we use if it is reasonable to assume that there are no individual effects that are independent of time and also no effect of time on all the individuals?",
    answer("Fixed effects", message = "Fixed effects model assumes that there are unknown or unobserved unique aspects about the individuals or heterogeneity among individuals that are not explained by the independent variables but influence the outcome variable of interest."),
    answer("Random effects", message = "Random effects model assumes that there are unknown or unobserved unique qualities about the individuals that influence the outcome variable of interest that are not correlated with the independent variables."),
    answer("Independently pooled panels", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
```{r, echo = FALSE}
load("www/data/DONOHUE_simulations.rda")
```
...
```{r, echo = FALSE}
load("www/data/LOTT_simulations.rda")
```
...
``` {r MA_Quiz, echo = FALSE}
quiz(caption = "",
  question("Multicollinearity often occurs when indivadual obervations are highly related to one another.",
    answer("True", message = "Multicollinearity often occurs when **independent variables** are highly related to one another."),
    answer("False", correct = TRUE, message = "Multicollinearity often occurs when **independent variables** are highly related to one another."),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  question("Which of the following methods can be helpful to assess if there may be multicollinearity? (select all that apply)",
    answer("Look at correlation between variables using scatter plots", correct = TRUE),
    answer("Look at correlation between variables using heatmaps", correct = TRUE),
    answer("Look at stability of the coefficient estimates", correct = TRUE),
    answer("Calculate the variance inflation factor (VIF)", correct = TRUE),
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
)
...
Use `ggplot` to make a plot that writes
$$ax^2 + bx + c = 0 \xrightarrow{\text{solve}} x = \frac{-b \pm \sqrt{b^2-4ac}}{2a}$$
vertically.
