...
knitr::opts_chunk$set(
  ...
  message = FALSE, warning = TRUE, cache = FALSE, fig.width = 10, fig.height = 7,
...
```{r, echo=FALSE}
rt <- suppressWarnings(wordcountaddin::readability("index.Rmd", quiet=TRUE)) # producing readability markdown table
...
knitr::include_graphics("https://tile.loc.gov/storage-services/service/rbc/rbpe/rbpe24/rbpe244/24404400/001dq.gif")
```

##### [[source]](https://www.loc.gov/item/rbpe.24404400/){target="_blank"}
...
comparing_analyses_plot <- ggplot(comparing_analyses) +
  ...
  annotate("segment", x = 1, y = 0.125, xend = 1, yend = 0.175,
  ...
  annotate("segment", x = 2, y = -0.125, xend = 2, yend = -0.175,
    arrow = arrow(angle = 45, ends = "last", type = "open"),
    linewidth = 2,
    color = "red",
    lineend = "butt",
...
Recall that the only difference between the two models is the number of demographic variables included as covariates. The number of rows or observations is the same, as are the outcome and the other covariates included in the model. We can use the base R `all.equal()` function to compare the Donohue-like data and our Mustard and Lott-like data.

```{r}
all.equal(DONOHUE_DF, LOTT_DF)
```
...
RTC_LAW +
  ...
  Population_log +
  police_per_100k_lag,
data = data.frame(DONOHUE_subsets[[1]]),
index = c("STATE", "YEAR"),
model = "within",
effect = "twoways"
)
...
vif_plot <- vif_df %>%
  ...
  annotate("text", x = .7, y = 18, label = "typical cutoff of 10") +
...
Let's make this plot a little easier to see using the `coord_transform()` function of the `ggplot2` package with the `y ="log10"` argument. This does not change the values, but adjusts the way the y axis is displayed with diminishing distance between grid lines. 
...
vif_plot <- vif_df %>%
  ...
  geom_jitter(width = 0.1, alpha = 0.5, size = 2) +
  geom_hline(yintercept = 10, color = "red") +
  annotate("text", x = .75, y = 13, label = "typical cutoff of 10") +
...
plot_B1 <- ggplot(empty_df, aes(x = X, y = Y)) +
  ...
  ylim(0, 10) +
  xlim(0, 10) +
  annotate("segment", x = 5, y = 6, xend = 5, yend = 4.5,
    arrow = arrow(angle = 45, ends = "last", type = "open"),
    linewidth = 1.8,
    color = "black",
    lineend = "butt",
