...
avocado - how does this look after I explained it more... the reference above seems to do VIF for a oneway model ... I tried to enforce twoways with the outcome variable but it doesn't seem to change the result (which might be a good thing?). Reply from MAT: Yeah, I was not sure about the oneway vs twoway difference, but the coefficient estimate is the same in any case. So I had added a little language to hedge ("some idea of the VIF") and hope that it will be OK. 
...
The coefficients from this model then are an oversimplifed explanation centered across all states and across all time points. This is called "demeaned" data.
...
vif_plot <- vif_df %>%
  ...
  geom_hline(yintercept = 10, color = "red") +
  scale_y_continuous(trans ="log10") +
  labs(title = "Variance inflation factors", y= "log10 VIF") + 
  theme_minimal() +
  theme(axis.title.x = element_blank(),
         axis.text.x = element_text(color = "black"),
         axis.text.y = element_text(color = "black"))
