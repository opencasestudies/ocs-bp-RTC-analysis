...
Hmm, bummer this is not in PMC for a free version....
...
AVOCADO: perhaps change main question to What is the effect of multicollinearity on coefficient estimates from linear regression models when analyzing right to carry laws and violence rates?
...
1. Provide an illustration of what multicollinearity is and how it can influence linear regression coefficients.
2. Demonstrate how to look for the presence of multicollinarity and determine its severity.
3. Illustrate the difference between multicollinearity and correlation.
4. Provide tools and guidance on the implementation of panel regression analysis (`plm` package).
5. Define and show how to calculate a variance inflation factor (VIF) (`car` package).
...
AVOCADO: Should these data science learning objectives be updated now that we have split the case studies? I know some  of these are covered in the other one (I think items 1-5), so perhaps we can indicate that with a different  font, or an asterisk and refer the reader to the other case study?
...
AVOCADO: Change "efficient" to whatever Mike Love suggested?
...
AVOCADO: I put this next section in a details clickable box. My sense is that we should delete this information entirely from the case study, but keeping it for now and will come back and decide at the end if we want to keep it after I look through the whole thing. I agree this part can be dropped (MAT)

<details> <summary> Click here for more information on right-to-carry policies over time. </summary>
...
I have tried editing it some more, but I agree that we should have someone else take a look at it as well. (MAT)

4) It is important to note that we do not treat race as an objective measure. While we recognize the limitations around determining and reporting race, and interpretation of any observed difference by race, we believe including race as an explanatory variable in an epidemiological study can be useful to advance scientific inquiry. For more information on this topic, we have included a link to a [paper on the use of race as a measure in epidemiology](https://academic.oup.com/epirev/article/22/2/187/456942). 
...
[scales](https://cran.r-project.org/web/packages/scales/scales.pdf){target="_blank"}    | to allow us to look at the colors within the viridis package and to override some ggplot2 defaults for scales and legends
...
[rsample](https://rsample.tidymodels.org){target="_blank"} | a tidyverse package for easily resampling data 
...
See [this case study](needlink){target="_blank"} for details about data import and wrangling. We include data sources here for completeness, but will start from processed data in this case study.
...
The final products from our data wrangling are two tibbles of data each with variables selected to be similar to those used in either the Donohue or Mustard and Lott analysis. The overarching idea of this case study is to compare the model results from these two data sets, which differ only in the demographic variables. This is an oversimplification of the actual differences between the data sets and approaches taken by Donohue and Lott/Mustard, but will be useful for illustrating the impact of multi-collinearity on our modeling results, which is the main objective of this case study.
...
AVOCADO: is there another good motivation for converting the units like this? I think it is worth explaining as transformations/interpretability are important components of data science.
...
3) **random effects** - assumes that there are unknown or unobserved unique qualities about the individuals that influence the outcome variable of interest that are **not correlated** with the independent variables. 

Thus, the random effects model actually makes **more assumptions** than the fixed effect model.

$$Y_{it} =\beta_0 X_{it} +\beta_{1}X_{1it}+... \beta_k X_{kit} + e_{t}$$
...
There is one more test that we could perform. To test if using a random effect model would be more appropriate compared to the fixed effect model, one could use the [Hausman test](https://en.wikipedia.org/wiki/Durbin%E2%80%93Wu%E2%80%93Hausman_test){target="_blank"} for this (also called the Durbin-Wu-Hausman test). This can be implemented using the `phtest()` function of the `plm` package.
...
Another way to look at correlation if we have many variables is to use heatmaps where the intensity of the color indicates the strength of the correlation between two variables.
...
We will add a legend title to include $\rho$ by using the base `expression()` function, which will convert the written form of `"rho"` to the Greek symbol.
...
AVOCADO: Is it possible to reorder the variables so they are more consistent between the two panels? I would like the variables that are exactly shared to be grouped together on one side or the other in the same order, and the demographic variables grouped together, more or less in the same order (i.e., black, white and then other, for example).
...
AVOCADO: What terms are shown in this plot? All of them? A particular one?
...
AVOCADO: Is there a good reference for the above statement?
...
AVOCADO: When I read that entry, it looks like as long as you are using model="pooling" for the plm call, you are OK (not that I really know what is going on here -- I'll try to figure it out more)
...
AVOCADO: change y-axis label  to log VIF
